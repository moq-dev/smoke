...
#
#   - rust    : `moq auth` from the moq CLI (cargo / brew / apt / nix), on PATH
...
# The moq CLI under test (its `auth` subcommand). Whatever channel installed it
# (cargo/brew/apt/nix) just has to leave it on PATH; TOKEN_BIN (or MOQ_BIN, shared
# with smoke.sh) points at a specific build.
TOKEN="${TOKEN_BIN:-${MOQ_BIN:-moq}}"
...
cli_for() {
    # The command prefix that runs each implementation's auth CLI. The word
    # split is deliberate (runtime + path, or a whole `docker run ...` line), so
    # callers expand it unquoted.
    case "$1" in
        rust) echo "$TOKEN auth" ;;
...
gen() {
    ...
    # Every CLI shares the same generate flags; JS also echoes the key to stdout.
    if [[ "$algo" == HS* ]]; then
        # shellcheck disable=SC2086  # cli is a deliberate multi-word prefix
        $cli generate --algorithm "$algo" --out "$dir/sign.jwk" >/dev/null
        cp "$dir/sign.jwk" "$dir/verify.jwk"
    else
        # shellcheck disable=SC2086
        $cli generate --algorithm "$algo" --out "$dir/sign.jwk" --public "$dir/verify.jwk" >/dev/null
    fi
}
...
if needs rust; then
    ...
    # mid-matrix.
    elif "$TOKEN" auth generate --algorithm HS256 --out "$TMP/rust-probe.jwk" >"$TMP/rust-probe.log" 2>&1; then
        echo "rust:    $(command -v "$TOKEN") auth"
    else
