...
relays.sh                orchestrator: smoke.sh matrix through the public relays in the interop-runner registry
report/                  relays.sh's HTML report (index.html) + build.sh, published to GitHub Pages nightly
...
| `rust-docker` | the [`moqdev/moq`](https://hub.docker.com/r/moqdev/moq) Docker Hub image (`:latest`) | `docker run moqdev/moq auth …` |
...
`docker pull`s the `moqdev/moq` image fresh (`:latest`) and runs the CLI in a throwaway container with the scratch
...
just token-full       # full matrix: rust, js-node, js-bun + rust-docker (the
                      # moqdev/moq image, where a container runtime is
...
- **Docker channel** (`moqdev/moq-relay` + `moqdev/moq`, Linux): working. The containerised relay routes the full matrix and the containerised `moq` publishes/subscribes end-to-end, validated against the published images.
...
- **External relays** (`relays.sh`): moq-dev's `cdn.moq.dev` (required) passes over WebTransport and raw QUIC, for both the Rust and browser clients. Among the optional relays, **moqx** routes rust/browser media over both transports (except browser subscribe to a Rust publisher, which failed on the one run so far), and the Nokia relay passed rust → rust once. **Cloudflare's draft-18 relay** accepts the session and the publisher's `PUBLISH_NAMESPACE`, but never answers the subscriber's root `SUBSCRIBE_NAMESPACE` (an empty prefix is legal from draft-16 on), so the announcement is never forwarded and no media flows. On draft-14/15 relays (Cloudflare's draft-14 one) that empty prefix is itself a protocol violation, which is on moq-net. **Cloudflare's draft-16 relay** (`draft-16.cloudflare.mediaoverquic.com`, not in the registry yet, so `relays.sh` carries it as an extra endpoint) negotiates `moq-transport-16`, but over WebTransport the session fails with a protocol violation right after the publisher's `PUBLISH_NAMESPACE`, and over raw QUIC the relay closes the connection with "scope resolution failed". moxygen's public endpoint currently serves an expired certificate, quiche-moq times out, libquicr rejects the offered versions, and imquic / moqtail / stitcher connect but deliver no data. These rows are informational: they're how we find out which side needs the fix.
